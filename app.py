...
def dashboard():
    ...
    for mn in set(income_map) | set(expense_map):
        ...
        exp = expense_map.get(mn, 0)
        net_summary.append({'month': mn, 'billed': billed, 'expenses': exp, 'net': billed - exp})
...
def readings():
    ...
    if request.method == 'POST':
        ...
        raw_date = request.form.get('reading_date', '').strip()
        if raw_date:
            reading_date = datetime.strptime(raw_date, '%Y-%m-%d').strftime('%d %b %Y')
        else:
